...
it, so an LLM might actually be able to summerise some key commands for you. 
Do actually practice their use though.
...
## A0 Prep
As preparation for A0, we're going to have a go at implementing some functions 
used in A0. The purpose here is to get some familiarity with what is going on
...
using a reduced database from that used in A0 so there are fewer attributes 
